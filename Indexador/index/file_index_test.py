...
class FileIndexTest(unittest.TestCase):
    ...
    def check_idx_file(self, obj_index, set_occurrences):
        ...
        list_size = obj_index.idx_tmp_occur_last_element - \
            obj_index.idx_tmp_occur_first_element + 1
        self.assertEqual(
            list_size, 0, "A lista de ocorrencias deve ser zerada após chamar o método save_tmp_occurrences")
    ...
    def test_next_from_file(self):
        ...
        with open("term_test", "rb") as file:
            ...
            self.assertEqual(
                occur1, occur1_read, f"Primeiro elemento deveria ser {occur1} porém foi obtido {occur1_read}")
            self.assertEqual(
                occur2, occur2_read, f"Segundo elemento deveria ser {occur2} porém foi obtido {occur2_read}")
            self.assertIsNone(
                occur3_read, "Não há 3o elemento, assim, deveria retornar None na terceira leitura")
    ...
    def test_next_from_list(self):
        ...
        self.assertEqual(self.index.get_tmp_occur_size(),
                         0, f"Tamanho incorreto da lista")
        ...
        self.assertEqual(self.index.get_tmp_occur_size(),
                         4, f"Tamanho incorreto da lista")

        for i in range(4):
            next_term = self.index.next_from_list()
            self.assertEqual(
                next_term, terms_to_add[i], f"next_from_list deveria remover o termo {terms_to_add[i]} e foi o {next_term} ")
            self.assertEqual(self.index.get_tmp_occur_size(
            ), 3-i, f"Tamanho incorreto da lista ao remover o elemento #{i}")
        next_term = self.index.next_from_list()
        self.assertEqual(self.index.get_tmp_occur_size(
        ), 3-i, f"Após a remoção de todos os elementos da lista, o tamanho deveria ser vazio")
        self.assertIsNone(
            next_term, "Após a remoção de todos os elementos da lista, o metodo next_from_list deveria retornar none")
